...
class HybridRAG:
    ...
    async def query(
        ...
        has_scope = bool(document_ids or collection_ids)
        effective_top_k = top_k or self.config.SIMILARITY_TOP_K
        ...
        trace = {
            ...
            "cache": {
                "enabled": False if (not cache or has_scope) else self.config.USE_CACHE,
                "hit": False,
            },
        ...
        try:
            ...
            use_cache = False if (not cache or has_scope) else self.config.USE_CACHE
            if has_scope and cache:
            ...
            with stage_timer("retrieval", logger, trace_id, metrics):
                retrieved_nodes = await self.retriever.retrieve(
                    query,
                    self.vector_store_provider.supports_sparse(),
                    document_ids=document_ids,
                    collection_ids=collection_ids,
                    top_k=effective_top_k,
                )
